...
class PVae(VAE):
    def make_networks(self, obs_shape, z_dim, code_dim):
        self.enc = nn.Sequential(nn.Conv2d(obs_shape[0], 32, 3, stride=2),
                                     ...
                                     nn.ReLU(), nn.Conv2d(32, 32, 3, stride=1),
                                     nn.ReLU(), nn.Conv2d(32, 32, 3, stride=1),
                                     nn.ReLU(), nn.Flatten(), nn.Linear(32 * 35 * 35, 150),
                                     nn.ReLU())
        self.enc_mu = nn.Linear(150, code_dim)
        self.enc_logvar = nn.Linear(150, code_dim)
        self.dec = nn.Sequential(nn.Linear(code_dim, 32 * 35 * 35),
                                    nn.ReLU(), nn.Unflatten(dim=1, unflattened_size=(32, 35, 35)),
                                    nn.ConvTranspose2d(32, 32, 3, stride=1),
                                    nn.ReLU(), nn.ConvTranspose2d(32, 32, 3, stride=1),
                                    nn.ReLU(), nn.ConvTranspose2d(32, 32, 3, stride=1),
                                    nn.ReLU(), nn.ConvTranspose2d(32, 32, 3, stride=1),
                                    nn.ReLU(), nn.ConvTranspose2d(32, 32, 3, stride=2),
                                    nn.ReLU(), nn.Conv2d(32, obs_shape[0], 4, stride=1))
...
class PSMM(nn.Module):
    def __init__(self, obs_shape, z_dim, hidden_dim, vae_beta, device):
        super().__init__()
        self.z_dim = z_dim
        self.vae = PVae(obs_dim=obs_shape, z_dim=z_dim, code_dim=128, vae_beta=vae_beta, device=device)
        self.apply(utils.weight_init)
...
class SMMAgent(DDPGAgent):
    def __init__(self,
                 ...
                 sp_lr,
                 vae_lr, vae_beta,
                 state_ent_coef, latent_ent_coef, latent_cond_ent_coef, update_encoder,
                 **kwargs):
        self.z_dim = z_dim
